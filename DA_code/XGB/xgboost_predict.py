...
# def process_data(arrays_list):
#     y = np.stack(arrays_list, axis=0)
#     print('y shape=',y.shape)
#     num_,nl_,ns_ = y.shape
#     feature_li = y.reshape(nl_*ns_,num_)
#     del y
#     mask = feature_li[:,1].copy()
#     feature_arr = np.array(feature_li)
#     del feature_li
#     feature_arr_s = feature_arr
#     feature_arr_s[np.isinf(feature_arr_s)] = -1
...
def xgboost_predict_s(model_path, data_path, out_path, feature_name, ii):
    data_out = f'{out_path}/xgboost_predict_{str(ii + 1).zfill(2)}.tif'
